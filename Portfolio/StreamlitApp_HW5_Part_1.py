...
MODEL_INFO = {
        ...
        "keys": ["OXY_CR_Cum","FANG_CR_Cum"], 
        "inputs": [{"name": k, "type": "number", "min": -100.0, "max": 100.0, "default": 0.0, "step": 10.0} for k in ["OXY_CR_Cum","FANG_CR_Cum"]] 
}
...
def display_explanation(input_df, session, aws_bucket):
    ...
    st.pyplot(fig)
    # top feature 
    top_feature = pd.Series(shap_values[0, :, 0].values, index=shap_values[0].feature_names).abs().idxmax()
